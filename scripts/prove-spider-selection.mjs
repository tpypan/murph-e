...
try {
  ...
  for (const players of [1, 2]) {
    const result = await page.evaluate(
      ({ code, players }) => {
        const p = window.__probe,
          load = p.load(code, 19, 'SPIDER-MAN: ROOFTOP WEB DUEL', players)
    ...
    assert.equal(
      get('p1-batman').state.selection.available[get('p1-batman').state.selection.cursors[0]],
      'batman',
    )
    if (players === 2) {
      ...
      assert.ok(get('fight').text.includes('SPIDER-MAN'))
    } else {
      assert.equal(get('unlocked').state.selection.humans, 1)
      assert.equal(get('p1-locked').state.phase, 'versus')
      assert.equal(get('fight').state.fighters[0].id, 'batman')
      assert.notEqual(get('fight').state.fighters[1].id, 'batman')
  ...
  console.log('Actual demo1P/2P selection, independent locks, identity labels and fight passed')
} finally {
  await browser.close()
}
