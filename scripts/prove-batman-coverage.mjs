...
const root = resolve(import.meta.dirname, '..'),
  ...
  out = process.argv.includes('--adapter')
    ? resolve(root, 'data/local-catalog/batman-atari-fighter-reference/evidence/coverage')
    : resolve(cache, 'complete-coverage'),
  module = readFileSync(
    resolve(
      root,
      process.argv.includes('--adapter')
        ? 'data/local-catalog/batman-atari-fighter-reference/module.js'
        : 'library/catalog/fighter/module.js',
    ),
    'utf8',
  ),
  assets = { 'batman-atari-reference': JSON.parse(readFileSync(resolve(cache, 'actor.json'))) },
...
try {
  ...
  for (const owner of [0, 1])
    for (const mode of [
      ...
      const roster =
          owner === 0 ? ['batman-atari-reference', 'flash'] : ['flash', 'batman-atari-reference'],
        config = {
          ...(process.argv.includes('--adapter') ? {} : { assets }),
          roster,
          names: owner === 0 ? ['BATMAN', 'FLASH'] : ['FLASH', 'BATMAN'],
          roundsToWin: 1,
      ...
      const result = await page.evaluate(
        ({ code, owner, mode }) => {
          const p = window.__probe,
            r = p.load(code, 19, 'BATMAN SOURCE DUEL', 2)
