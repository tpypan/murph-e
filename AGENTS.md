...
5. **The cabinet never dead-ends.** Generation failures preserve the current game and
...
- Prompt or model change: use offline prompt/retrieval/stream tests and frozen runs.
  The following historical paid bench commands are blocked for development:
  `pnpm harness bench bench/prompts.txt` and compare
