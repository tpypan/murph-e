const PLANE_PALETTE = [
  "#182747", "#ce354a", "#ff6570", "#fff1d0",
  "#c6bca9", "#2469b3", "#71dafa", "#ffffff"
];
const PLANE = [
  "...........33...........",
  "..........3773..........",
...
const COURSE_NAMES = ["CORAL COAST", "CLOUD GARDEN", "TEMPEST PASS"];
const LEVEL_NAMES = ["LOW", "MID", "HIGH"];
const ITEM_NAMES = ["GUST", "BALLOON", "PUFF"];
const RACER_NAMES = ["RED COMET", "GOLD WING", "GREEN JET", "VIOLET"];
...
function centerAt(s) {
  ...
  return 128 + fade * (Math.sin(d / 290 + race * 0.55) * (12 + race * 8)
    + Math.sin(d / 117) * 6);
}
function widthAt(s) {
  return 190 - race * 5 - 8 * Math.pow(Math.sin(s / 410), 2);
}
function screenY(s) {
...
function burst(x, s, color, count) {
  for (let i = 0; i < count; i++) {
    ...
    particles.push({
      x: x, s: s, vx: (Math.random() - 0.5) * 80,
      vs: (Math.random() - 0.5) * 110,
      life: 0.35 + Math.random() * 0.4, color: color
    });
  }
}
function addObject(type, s, x, alt, extra) {
  const o = {
    type: type, s: s, x: x, alt: alt, done: false,
    cool: 0, move: 0, aiMask: 0, pending: 0, serial: -1
  };
  if (extra) {
  ...
  for (let lap = 0; lap < 3; lap++) {
    ...
    for (let k = 0; k < 5; k++) {
      ...
      addObject("gate", s, centerAt(s), -1, {
        half: k === 0 ? 73 : 57 - race * 2, number: k + 1
      });
    }
    ...
    addObject("balloon", base + 1080, centerAt(base + 1080) + 43, 2,
      { move: 9 + race * 3 });
    addObject("cloud", base + 1180, centerAt(base + 1180) - 29, 0,
      { move: 12 + race * 4 });
    ...
    if (race > 0 || lap > 0) {
      addObject("wind", base + 985, centerAt(base + 985), -1,
        { length: 165, dir: (lap + race) % 2 ? -1 : 1 });
      addObject("cloud", base + 1490, centerAt(base + 1490) + 32, 1,
        { move: 20 });
    }
    if (race === 2) {
      addObject("balloon", base + 490, centerAt(base + 490) - 40, 2,
        { move: 15 });
    }
  }
  objects.sort(function(a, b) { return a.s - b.s; });
}
function startRace(api) {
  ...
  racePlace = 1;
  player = {
    x: 128, s: 0, vx: 0, alt: 1, visualAlt: 1,
    item: 0, boost: 0, cooldown: 0, spin: 0, inv: 3.1,
    spins: 0, crash: 0, checkpoint: 0, assist: 0,
    time: 0, finished: false
  };
  rivals = [];
  for (let i = 0; i < 3; i++) {
    rivals.push({
      id: i + 1, x: 75 + i * 52, s: 23 + i * 17,
      alt: i, slow: 0, fog: 0, hurt: 0,
      baseSpeed: 134 + i * 4 + race * 2,
...
function updateRivals(api, dt) {
  for (let i = 0; i < rivals.length; i++) {
    ...
    for (let j = 0; j < objects.length; j++) {
      ...
      if ((o.type === "cloud" || o.type === "balloon") &&
          o.alt === r.alt && Math.abs(objectX(o, api.t) - target) < 48) {
    ...
    for (let j = 0; j < objects.length; j++) {
      ...
      o.aiMask |= 1 << i;
      if ((o.type === "cloud" || o.type === "balloon") &&
          r.alt === o.alt && Math.abs(r.x - objectX(o, api.t)) < (o.type === "cloud" ? 37 : 22)) {
        r.slow = 1.2;
        r.hurt = 0.6;
...
function update(api, dt) {
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    } else if (o.type === "crate" && close && dx < 22 && player.alt === o.alt &&
               o.cool <= 0 && player.item < 0 && player.spin <= 0) {
    ...
    } else if ((o.type === "cloud" || o.type === "balloon") && close &&
               dx < (o.type === "cloud" ? 36 : 21) && player.alt === o.alt) {
...
function drawRival(api, r, index) {
  ...
  if (r.hurt > 0 && api.frame % 6 < 3) {
    ...
    if (index !== 1) api.line(x - (api.frame % 4 < 2 ? 7 : 3), y - 9,
      x + (api.frame % 4 < 2 ? 7 : 3), y - 9, 7);
...
function drawPlayer(api) {
  ...
  if (player.crash > 0) {
    const a = api.t * 9;
    api.line(x - Math.cos(a) * 15, y - Math.sin(a) * 15,
      x + Math.cos(a) * 15, y + Math.sin(a) * 15, 8);
    api.line(x + Math.sin(a) * 10, y - Math.cos(a) * 10,
      x - Math.sin(a) * 10, y + Math.cos(a) * 10, 15);
    api.circfill(x, y, 5 + api.frame % 4, 9);
    api.circfill(x, y, 3, 10);
    for (let i = 0; i < 4; i++) {
      api.circfill(x + Math.sin(api.t * 4 + i) * 8,
        y - 8 - i * 9, 5 + i, i % 2 ? 5 : 6);
    }
  ...
  if (player.spin > 0) {
    const a = api.t * 17;
    api.line(x - Math.cos(a) * 14, y - Math.sin(a) * 14,
      x + Math.cos(a) * 14, y + Math.sin(a) * 14, 15);
    api.line(x - Math.cos(a) * 13, y - Math.sin(a) * 13 + 1,
      x + Math.cos(a) * 13, y + Math.sin(a) * 13 + 1, 8);
    api.line(x - Math.sin(a) * 9, y + Math.cos(a) * 9,
      x + Math.sin(a) * 9, y - Math.cos(a) * 9, 8);
    api.circfill(x, y, 3, 6);
    api.circ(x, y, 19, 7);
  } else {
    const banking = Math.abs(player.vx) > 32;
    api.spr(banking ? BANK : PLANE, x - 12, y - 8,
      banking && player.vx > 0, false, PLANE_PALETTE);
...
function drawHUD(api) {
  ...
  api.text("R" + (race + 1) + " LAP " + Math.min(3, Math.floor(player.s / LAP_LENGTH) + 1) + "/3", 5, 15, 7);
  api.text("" + racePlace + "/4", 120, 15, 10);
  const seconds = Math.floor(elapsed);
  const timeText = Math.floor(seconds / 60) + ":" + (seconds % 60 < 10 ? "0" : "") + seconds % 60;
  ...
  api.text("A:" + (player.item < 0 ? "EMPTY" : ITEM_NAMES[player.item]), 4, 214, player.item < 0 ? 6 : 7);
  ...
  if (finishY > 39 && finishY < 209) {
    for (let x = 37; x < 222; x += 9) {
      for (let row = 0; row < 2; row++) {
        api.rectfill(x, finishY + row * 7, 9, 7,
          (Math.floor((x - 37) / 9) + row) % 2 ? 1 : 7);
      }
    }
