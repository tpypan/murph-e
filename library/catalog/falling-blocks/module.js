(config = {}) => {
  ...
  const clearFlash = config.clearFlash && typeof config.clearFlash === 'object' ? {
    color: Number.isFinite(config.clearFlash.color) ? clamp(Math.floor(config.clearFlash.color), 0, 15) : 10,
    seconds: Number.isFinite(config.clearFlash.seconds) ? clamp(config.clearFlash.seconds, 0.1, 1) : 0.35,
  } : null;
  ...
  function init(api) {
    ...
    boards = Array.from({ length: players }, (_, index) => {
      const b = { index, grid: Array.from({ length: H }, () => Array(W).fill(0)), next: [], bag: [], rng: (config.seed >>> 0) || 7,
        ...
        pending: 0, dropFlash: 0, dropCells: [], flash: 0, clearPulse: 0,
  ...
  function lock(api, b) {
    ...
    if (b.clearing.length) { b.clearTimer = 0.2; if (clearFlash) b.clearPulse = clearFlash.seconds; api.sfx('coin'); }
  ...
  function updateBoard(api, b, dt) {
    ...
    b.clearPulse = Math.max(0, b.clearPulse - dt);
    if (b.clearTimer > 0) {
  ...
  function layout() {
    const solo = players === 1, cell = solo ? 9 : 8, y = solo ? 24 : 34;
    return {
      scoreHud: { x: 0, y: 0, w: 256, h: 12 },
      controls: { x: 0, y: 210, w: 256, h: 14 },
      sharedHud: solo ? [{ x: 0, y: 12, w: 72, h: 198 }, { x: 168, y: 12, w: 88, h: 198 }] : [{ x: 100, y: 12, w: 56, h: 198 }],
      wells: boards.map((b) => {
        const x = solo ? 74 : b.index ? 161 : 15;
        return { player: b.index, cell, field: { x, y, w: W * cell, h: H * cell },
          border: { x: x - 2, y: y - 2, w: W * cell + 3, h: H * cell + 3 },
          next: { x: solo ? 187 : b.index ? 132 : 106, y: 44, w: solo ? 32 : 16, h: solo ? 96 : 64 } };
      }),
    };
  }
  function draw(api) {
    ...
    for (const b of boards) {
      const well = geometry.wells[b.index], cell = well.cell, x0 = well.field.x, y0 = well.field.y, pc = b.index ? 8 : 12;
      api.rect(well.border.x, well.border.y, well.border.w, well.border.h, clearFlash && b.clearPulse > 0 ? clearFlash.color : pc);
