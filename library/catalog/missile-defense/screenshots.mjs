...
try {for(const players of [1,2]){
 ...
 const load=await page.evaluate(({code,players})=>window.__probe.load(code,7,'SKY SHIELD',players),{code,players});assert.equal(load.ok,true,load.error)
 await page.evaluate(()=>window.__probe.start());let previous=0
 for(const [name,frames] of [["ready", 1], ["aim", 180], ["blast", 500], ["wave-3", 2100], ["finished", 9000]]){
